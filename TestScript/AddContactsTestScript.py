...
wb = ExcelUtil(test_data_file_path)  #生成excel对象的实例
def get_test_data(sheet_name):
    global wb
    wb.set_sheet_by_name(sheet_name)    #设定要操作的sheet
    user_info = wb.get_sheet_all_data()
    return user_info


def run_test_case():
    user_info = get_test_data(test_user_info_sheet)  # 获取账号登录的所有数据
    ...
    for line in user_info[1:]:
        ...
        if "y" in line[execute_flag_col_no]:
            info("此用户数据要被执行：%s,%s,%s" % (
            line[username_col_no], line[password_col_no], line[test_data_sheet_name_col_no]))
            ...
            add_contacts_data = get_test_data(test_data_sheet)  # 获取测试结果的所有数据
            print("---:", add_contacts_data)
            driver = login(username, password)  # 调用一下登录逻辑
            wb.set_sheet_by_name(test_result_sheet)  # 获取测试结果表格操作对象
            wb.write_a_line_in_sheet(add_contacts_data[0], fgcolor="CD9B9B")  # 在测试结果页第一行写了一个表头
            flag = True
            for i in add_contacts_data[1:]:  # 遍历所有新建联系人的数据，在登录状态下进行新建操作
                ...
                if "y" in i[test_data_execute_flag_col_no]:
                    ...
                    info("当前联系人数据行：%s,%s,%s,%s,%s," % (
                        contact_name, contact_email, contact_star, contact_mobile, contact_comment))
                    try:
                        add_person_info(driver, contact_name, contact_email, contact_star, contact_mobile,
                                        contact_comment)
                        ...
                        info(traceback.format_exc())
                        # traceback.print_exc()
                        i[test_data_result_no] = "其他异常失败"  # 设定一下测试结果列中的状态
                        take_pic(driver)
                        flag = False
                        info("当前测试数据执行出现了异常失败")
